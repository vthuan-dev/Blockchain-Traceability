window.StatManager = {
  ...
  updateUserCount() {
    this.getUserCount().then((count) => {
      const userCountElement = document.querySelector(
        ".stat-card-custom.bg-green .stat-info p"
      );
      if (userCountElement) {
  ...
  updateProductCount() {
    this.getProductCount().then((count) => {
      const productCountElement = document.querySelector(
        ".stat-card-custom.bg-blue .stat-info p"
      );
  ...
  async renderUserStats() {
    ...
    try {
      const [statsResponse, provinceStatsResponse, registrationStatsResponse] =
        await Promise.all([
  ...
  createRegistrationChart(data) {
    ...
    document
      .getElementById("registrationPeriod")
