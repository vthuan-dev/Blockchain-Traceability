  ...
  window.RegionManager = {
        ...
        const row = `
                    <tr>
                        <td style="text-align: center; vertical-align: middle;">${
                          region.region_id || "N/A"
                        }</td>
                        <td style="text-align: center; vertical-align: middle;">${
                          region.region_name || "N/A"
                        }</td>
                        <td style="text-align: center; vertical-align: middle;">${
                          region.district_name || "N/A"
                        }</td>
                        <td style="text-align: center; vertical-align: middle;">${
                          region.ward_name || "N/A"
                        }</td>
                        <td style="text-align: center; vertical-align: middle;">${
                          this.formatDate(region.created_at) || "N/A"
                        }</td>
                        <td style="text-align: center; vertical-align: middle;">
    ...
    fetchRegionData: async function () {
      try {
        ...
        this.regionData = data.filter((region) =>
          region.region_id.startsWith(adminProvinceId)
        );
        ...
        showMessage(
          "Có lỗi xảy ra khi tải dữ liệu vùng sản xuất. Vui lòng thử lại sau.",
          "error"
        );
      }
    },
    ...
    deleteRegion: async function (regionId) {
      try {
        ...
        showMessage(
          "Có lỗi xảy ra khi xóa vùng sản xuất: " + error.message,
          "error"
        );
      }
    },
  ...
  if (searchInput) {
    searchInput.addEventListener(
      ...
      debounce(() => {
        ...
        window.RegionManager.filteredData =
          window.RegionManager.regionData.filter((region) =>
  ...
  if (nextButton) {
    nextButton.addEventListener("click", () => {
      const totalPages = Math.ceil(
        window.RegionManager.filteredData.length /
          window.RegionManager.rowsPerPage
      );
