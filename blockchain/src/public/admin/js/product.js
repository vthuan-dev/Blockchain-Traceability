    ...
    paginatedData.forEach((product) => {
      ...
      const row = `
                <tr>
                    ...
                    <td style="text-align: center; vertical-align: middle;">
                        <button class="action-icons bg-blue" title="Sửa" onclick="window.ProductManager.editProduct(${
                          product.product_id
                        }, '${escapeString(product.product_name)}', ${
        product.price
      }, '${escapeString(product.description)}', '${escapeString(
        product.uses
      )}', '${escapeString(product.process)}', '${escapeString(
        product.img
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function filterData() {
    ...
    window.ProductManager.filteredData =
      window.ProductManager.productData.filter((product) =>
  ...
  if (nextButton) {
    nextButton.addEventListener("click", () => {
      const totalPages = Math.ceil(
        window.ProductManager.filteredData.length /
          window.ProductManager.rowsPerPage
      );
