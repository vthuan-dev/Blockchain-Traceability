...
async function loadNotifications() {
    try {
        const response = await fetch('/api/inspector-notifications');
        if (response.ok) {
...
function createNotificationItem(notification) {
    ...
    notificationItem.innerHTML = `
        <div class="notification-content d-flex align-items-center p-3">
            <div>
                <div class="font-weight-bold mb-1">${notification.producer_name}</div>
                <div class="notification-text mb-1">
                    đã tạo lô hàng mới: ${notification.batch_name}
                </div>
...
function setupMarkAllReadButton(markAllReadBtn) {
    markAllReadBtn.addEventListener('click', async () => {
        try {
            ...
            // Cập nhật UI sau khi đánh dấu tất cả
